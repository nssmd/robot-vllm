...
其他入口：`/tools`、`/health`、`/readyz`、`/metrics`、`/recovery`。完整约定见 [API 文档](api.md)。
...
自动 GitHub CI 目前因发布凭据缺少 `workflow` 权限而未启用；[完整模板](../.github/ci-tests.yml)和[启用步骤](ci.md)已提供。

进一步阅读：[架构](architecture.md) · [部署与恢复](deployment.md) · [Pi 0.5 接入细节](pi05.md) ·
[VLA 扩展](vla.md) · [验证记录](validation.md) · [贡献指南](../CONTRIBUTING.md)。
