...
这不复用旧动作，也不减少逻辑输入 token；真实命中收益仍需在可用的端点配额下验证。
...
**正式机器人实验标准：** 新的操作性能与加速结论采用同一份冻结分布，默认
**10 个任务 × 每任务 10 个 seed × 每个条件**。下述滑块结果仅为历史诊断，
不能替代正式机器人操作评测。[正式实验协议](docs/ROBOT_EVALUATION_PROTOCOL.md)
